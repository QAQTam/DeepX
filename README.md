...
DeepX 是一款本地优先的 AI Agent 桌面应用：**WinUI3 原生桌面壳**（Rust + windows-reactor，无 WebView/Electron）+ Rust 后端 daemon + 原生 Windows 安装与更新组件。
...
- **WinUI3 原生桌面壳**（windows-reactor）：原生 XAML 视图族（侧栏/标题栏/ChatView/Composer/首页/技能/设置），零 Web 组件
...
```mermaid
graph LR
    Shell[DeepX.exe<br/>WinUI3 原生壳<br/>windows-reactor 视图族] -->|Ringing V1 HTTP/SSE| Daemon[deepx-daemon<br/>单实例守护进程<br/>127.0.0.1 随机端口]
...
- **原生直连**：壳的 `bridge.rs` 用 `deepx-client`（Ringing V1 HTTP/SSE 客户端）直连 daemon；事件经三频道 SSE 解析为原生视图状态（ChatView 事件泵 / 侧栏 / 标题栏 / 交互队列）。WebView、`deepx-bridge.js` 与 SolidJS renderer 已整体移除。
...
DeepX/
├── apps/
│   ├── winui/             # WinUI3 原生桌面壳（Rust + windows-reactor）
│   │   ├── src/           #   main/bridge/chat_view/header/sidebar/skills_view/shell_store
│   │   └── scripts/       #   构建、打包与 sidecar 准备脚本
...
壳启动时自动拉起 daemon（`launch_daemon_if_missing`）。
...
| `just build-winui` | 构建 WinUI3 壳（release） |
| `just package-winui-desktop` | 预置 daemon sidecar + 组装 winui 运行目录（完整安装包用） |
| `just winui-package` | 生成完整 Windows 安装包 |
| `just sfx-quick kind="full"` | SFX 快速拼接（staging 已就位时跳过构建） |
| `just check` | Rust workspace 静态检查 |
| `just test` | Rust 测试 |
| `just fmt` | Rust 格式检查 |
...
- 壳首屏并发请求由壳侧连接互斥（`bridge.rs`）收敛为单次 daemon 拉起，避免冷启动多实例并存。
...
该命令会同步 Rust workspace、根 `deepx-backend.lock.json`（含 Release manifest URL）与根 `package.json`。项目内置的版本审计脚本还会检查安装器及 Windows `DisplayVersion` 链路。
...
- **`/debug/` 调试页**：daemon 内置静态服务入口（端口见 `%USERPROFILE%\.deepx\daemon.json`）。SolidJS renderer 移除后该入口不再承载前端产物；保留用于服务任意静态调试目录（`DEEPX_DEBUG_RENDERER_DIR` 指定时）。
