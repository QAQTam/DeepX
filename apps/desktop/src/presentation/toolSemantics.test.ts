...
describe("toolArgsSummary（label 语义化）", () => {
    it("read 提取路径与行范围", () => {
    expect(toolArgsSummary("read_file", '{"path":"src/main.rs","start_line":10,"end_line":20}'))
      .toBe("src/main.rs L10-20");
    expect(toolArgsSummary("read_file", '{"path":"src/main.rs","line":5}')).toBe("src/main.rs L5");
    expect(toolArgsSummary("read_file", '{"path":"src/main.rs"}')).toBe("src/main.rs");
  });
    ...
    expect(toolArgsSummary("edit_file", '{"path":"src/a.ts"}')).toBe("src/a.ts");
    expect(toolArgsSummary("write", '{"file":"README.md"}')).toBe("README.md");
  });

    it("无有效参数时回退到第一个字符串参数或空串", () => {
    expect(toolArgsSummary("read_file", "{}")).toBe("");
...
describe("toolStatusLabel（状态词）", () => {
    it("ok → 已读取/已执行", () => {
    expect(toolStatusLabel("ok", "read_file", "src/main.rs")).toBe("已读取 src/main.rs");
    expect(toolStatusLabel("backgrounded", "exec", "npm test")).toBe("已执行 npm test");
  });
    it("error → 修改失败", () => {
    expect(toolStatusLabel("error", "edit_file", "src/a.ts")).toBe("修改失败 src/a.ts");
  });
    it("进行中 → 读取中/执行中", () => {
    expect(toolStatusLabel(undefined, "read_file", "src/main.rs")).toBe("读取中 src/main.rs");
    expect(toolStatusLabel("running", "exec", "npm test")).toBe("执行中 npm test");
  });
    it("无摘要时只显示动词", () => {
    expect(toolStatusLabel("ok", "read_file", "")).toBe("已读取");
