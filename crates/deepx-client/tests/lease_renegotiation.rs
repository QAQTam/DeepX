...
//! home + `DEEPX_DATA_DIR`), with a shortened lease TTL
//! (`DEEPX_TEST_LEASE_TTL_MS` = 3s, shorter than the client's 5s renewal
...
//!
//! Run with:  cargo test -p deepx-client --test lease_renegotiation -- --ignored
//! Requires a compiled daemon binary: cargo build -p deepx-daemon
...
use deepx_client::{Channel, ChannelStatus, Client, ClientHandlers, ClientOptions};
...
/// Spawn a private daemon with `DEEPX_TEST_LEASE_TTL_MS=3000` (lease expires
...
fn spawn_isolated_daemon(home: &PathBuf) -> Child {
    ...
    let child = Command::new(&daemon)
        .arg("run")
        .env("DEEPX_DATA_DIR", &data)
        .env("USERPROFILE", home)
        .env("HOME", home)
        .env("DEEPX_TEST_LEASE_TTL_MS", "3000")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("spawn daemon");
...
async fn lease_expiry_triggers_renegotiation_and_streams_recover() {
    ...
    unsafe {
        std::env::set_var("DEEPX_DATA_DIR", &data);
    }
