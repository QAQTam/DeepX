...
pub async fn handle_debug_http(
    ...
    if path == "/debug/__deepx_bridge__.js" {
        let body = format!(
            "window.__DEEPX_DEBUG__={{\"token\":\"{token}\",\"nonce\":\"{}\"}};\n",
            random_hex()
        );
        return write_static(
            &mut stream,
    ...
    if file.file_name().and_then(|n| n.to_str()) == Some("index.html") {
        ...
        let script = "<script src=\"./__deepx_bridge__.js\"></script>";
