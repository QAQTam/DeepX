...
impl SessionEngine {
    ...
    pub fn create(&self, agent: &mut crate::agent::AgentState, _cancel: &CancelToken) {
        ...
        agent.rebind_store();
        deepx_tools::bridge::set_runtime_context(&agent.session.seed, agent.config.permission_level);
    }
    ...
    pub fn resume(&self, agent: &mut crate::agent::AgentState, seed: &str, _cancel: &CancelToken) -> bool {
        ...
        if lifecycle::init_session(agent, Some(seed)) {
            ...
            log::info!("[SESSION] restored, {} turns (has_more={})", recent.len(), has_more);
            true
        } else {
    ...
    pub fn reload_config(&self, agent: &mut crate::agent::AgentState, _cancel: &CancelToken) {
        if let Ok(cfg) = deepx_config::Config::load() {
            ...
            if let Some(ref key) = cfg.context7_api_key {
                if !key.is_empty() { deepx_tools::bridge::set_context7_key(key); }
            }
