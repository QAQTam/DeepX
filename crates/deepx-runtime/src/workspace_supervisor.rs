//! WorkspaceSupervisor — daemon 拉起的 deepx-workspace serve 生命周期管理。
//!
//! daemon 启动时按 config `[workspace] mode` 拉起工具服务：
//!
//! - `local`：本机 `deepx-workspace serve`（与 daemon 同目录，Windows 原生）。
//! - `wsl`（仅 Windows）：经 `wsl.exe` 在 WSL 发行版内拉起
//!   `deepx-workspace serve`；WSL2 localhost 转发使 Windows daemon 经
//!   127.0.0.1 访问。WSL 不可用/未装 serve 时自动回退 local（降级记录）。
//!
//! 生命周期：随 daemon —— 拉起 → /health 就绪 → 崩溃退避重启（5s）→
//! daemon shutdown 时 kill 回收。worker 经 `DEEPX_WORKSPACE_URL/TOKEN`
...
impl WorkspaceSupervisor {
    ...
    fn try_start(mode: WorkspaceMode) -> Result<(Child, String, String, WorkspaceMode, String), String> {
        if mode == WorkspaceMode::Wsl {
            ...
            let probe = Command::new("wsl.exe")
                .args(["-e", "bash", "-lc", "command -v deepx-workspace"])
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn();
            match probe {
                Ok(mut p) => {
                    ...
                    if !ok {
                        return Err("wsl.exe probe failed: deepx-workspace not found in WSL (or WSL unavailable)".into());
                    }
                }
                Err(e) => {
        ...
        // 读取 stdout 的 `DEEPX_WORKSPACE_READY <host>:<port>` 行：
    ...
    fn spawn(mode: WorkspaceMode, token: &str) -> Result<Child, String> {
        let mut command = match mode {
            ...
            WorkspaceMode::Wsl => {
                // WSL 发行版内须已安装 deepx-workspace（PATH 可达）。
                let mut command = Command::new("wsl.exe");
                command.args([
                    "-e",
                    "bash",
                    "-lc",
                    "DEEPX_WORKSPACE_TOKEN=\"$DEEPX_WORKSPACE_TOKEN\" deepx-workspace serve --port 0",
                ]);
                command.env("DEEPX_WORKSPACE_TOKEN", token);
                command
            }
            ...
            match self.mode {
                ...
                WorkspaceMode::Wsl => {
                    let _ = Command::new("wsl.exe")
                        .args(["-e", "bash", "-lc", "pkill -f 'deepx-workspace serve'"])
                        .status();
                    let _ = child.kill();
                    let _ = child.wait();
                }
...
/// Windows 路径 → WSL /mnt 路径（`F:\DeepX` → `/mnt/f/DeepX`）。
...
pub fn diagnose_wsl() -> Result<serde_json::Value, String> {
    ...
    let (installed, workspace_version) = match run_wsl(
        ...
        Err(e) => {
            return Ok(serde_json::json!({
                ...
                "workspace_installed": false,
                "connection_ok": false,
                "error": format!("检查 deepx-workspace 失败: {e}"),
            }));
        }
    };
    ...
    if !installed {
        return Ok(serde_json::json!({
            ...
            "workspace_installed": false,
            "connection_ok": false,
            "error": "WSL 内未安装 deepx-workspace，请点击「安装到 WSL」",
        }));
    }
...
/// 安装 deepx-workspace 到 WSL（拷贝源码到 WSL 原生路径构建）。
///
/// - `repo_root`（可选）：Windows 侧仓库根（如 `F:\DeepX`）。缺失时尝试从
...
///   3. 产物 install 到 `~/.local/bin/deepx-workspace`；若 PATH 未含
///      `~/.local/bin` 则幂等追加到 `~/.bashrc`（否则 supervisor 的
///      `command -v deepx-workspace` 找不到）。
...
pub fn install_wsl(repo_root: Option<&str>) -> Result<serde_json::Value, String> {
    ...
    let repo_root = match repo_root {
        ...
        _ => match derive_repo_root() {
            ...
            None => {
                return Err(
                    "无法推导 DeepX 仓库路径：请在请求中提供 repo_root（Windows 路径，如 F:\\DeepX）"
                        .into(),
                );
            }
        },
    ...
    // 已安装 → 直接成功（幂等）。
    let (installed, _) = run_wsl(&["-e", "bash", "-lc", "command -v deepx-workspace"], 15)?;
    if installed {
        return Ok(serde_json::json!({
            "already_installed": true,
            "ok": true,
            "message": "WSL 内已存在 deepx-workspace",
        }));
    }
    ...
    let build_cmd = format!(
        "set -e; \
         ...
         cargo build --release -p deepx-workspace 2>&1 | tail -40; \
         test -x target/release/deepx-workspace && \
         mkdir -p ~/.local/bin && \
         install -m 755 target/release/deepx-workspace ~/.local/bin/deepx-workspace && \
         if ! command -v deepx-workspace >/dev/null 2>&1; then \
    ...
    let (ok2, out2) = run_wsl(
        &[
            ...
            "bash",
            "-lc",
            "~/.local/bin/deepx-workspace list 2>&1 | tail -1",
        ],
        15,
    )?;
    Ok(serde_json::json!({
...
mod tests {
    ...
    fn verify_local_accepts_matching_hash_and_rejects_mismatch() {
        let temp = std::env::temp_dir().join(format!("deepx-ws-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
        let exe = temp.join("deepx-workspace.exe");
    ...
    fn wsl_path_conversion_maps_drive_and_segments() {
        ...
        assert_eq!(
            to_wsl_path(r"\\?\C:\Users\qa\DeepX").as_deref(),
            Some("/mnt/c/Users/qa/DeepX")
        );
        // 非盘符路径（UNC / 相对）→ None
