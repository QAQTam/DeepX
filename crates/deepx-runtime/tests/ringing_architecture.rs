...
//!
//! 验证：
//! 1. domain crate 不依赖 legacy wire（deepx-proto）与 wire（deepx-ringing）——
...
fn domain_path_deps() -> Vec<String> {
    ...
    let manifest = std::path::Path::new(root)
        .join("../..")
        .join("crates/deepx-domain/Cargo.toml");
    let text = std::fs::read_to_string(&manifest).expect("deepx-domain Cargo.toml");
...
fn domain_crate_does_not_depend_on_legacy_or_wire() {
    ...
    assert!(
        !domain_deps.iter().any(|d| d.contains("deepx-proto")),
        "deepx-domain must not depend on deepx-proto (legacy), got {domain_deps:?}"
    );
    assert!(
        !domain_deps.iter().any(|d| d.contains("deepx-ringing")),
        "deepx-domain must not depend on deepx-ringing (wire), got {domain_deps:?}"
    );
}
