    ...
    <div class="context-panel">
      <button class="context-panel-trigger" onClick={() => { setOpen(!open()); if (!open()) refresh(); }} title="Context breakdown">
        ...
        <Show when={totalTokens() > 0}>
          <span class="context-token-badge">{totalTokens()}t</span>
        </Show>
      </button>
      <Show when={open()}>
        <div class="context-dropdown">
          ...
          <div class="context-dropdown-body">
            <Show when={stats() && totalTokens() > 0} fallback={<div class="context-empty">No context data yet. Send a message first.</div>}>
