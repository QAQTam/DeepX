...
export default function SkillsView(props: SkillsViewProps) {
  ...
  return <div class="skills-page">
    <div class="skills-header">
      <div>
        ...
        <div class="skill-runtime-summary">
          ...
          <span>catalog {props.catalogRevision?.slice(0, 8) || "-"}</span>
          <span>epoch {props.contextEpoch ?? 0}</span>
          <span>{props.tokenUsage ?? 0}/{props.tokenBudget ?? 0} tokens</span>
        </div>
      </div>
    ...
    <Show when={props.diagnostics?.length}>
      <details class="skill-diagnostics"><summary>{props.diagnostics!.length} diagnostics</summary>
...
function SkillCard(props: {
  ...
  const action = () => {
    switch (props.item.state) {
      ...
      case "requested": return <button disabled={props.disabled} onClick={() => props.onAction("release")}>Cancel</button>;
      ...
      case "review_due": return <><button disabled={props.disabled} onClick={() => props.onAction("retain")}>Retain</button><button disabled={props.disabled} onClick={() => props.onAction("release")}>Unload</button></>;
      default: return <button disabled={props.disabled} onClick={() => props.onAction("activate")}>Retry</button>;
    }
  };
  return <div class={`skill-row ${props.item.state}${props.pending ? " pending" : ""}`}>
    <div class="skill-row-main" onClick={props.onExpand}>
      ...
      <div class="skill-row-meta"><span class="skill-scope-badge">{props.item.scope}</span>
        <Show when={props.item.lease_remaining !== undefined}><span>lease {props.item.lease_remaining}</span></Show>
