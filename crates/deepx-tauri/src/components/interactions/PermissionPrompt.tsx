...
export default function PermissionPrompt(props: {
  ...
  return (
    <section class="interaction-prompt permission-prompt" aria-labelledby="permission-heading">
      ...
      <Show when={props.request.risk === "high" && props.request.paths.length > 0}>
        <label class="interaction-trust">
          <input type="checkbox" checked={trustFolder()} onChange={event => setTrustFolder(event.currentTarget.checked)} />
          信任此目录
        </label>
      </Show>
      <div class="interaction-actions">
        <button type="button" class="interaction-reject" data-reject disabled={busy()} onClick={() => respond(false)}>
          拒绝
        </button>
        <button
          type="button"
