...
let bridgeListener: Promise<UnlistenFn> | undefined;
let statusBridgeListener: Promise<UnlistenFn> | undefined;

async function ensureBridgeListener(): Promise<void> {
  if (!bridgeListener) {
    bridgeListener = tauriListen<ControlMessage>("backend-message", ({ payload }) => {
      if (payload.type === "event") dispatch(`agent-${payload.seed}-event`, payload.event);
  ...
  if (!statusBridgeListener) {
    statusBridgeListener = tauriListen("backend-status", ({ payload }) => dispatch("backend-status", payload));
  }
  await Promise.all([bridgeListener, statusBridgeListener]);
}
...
async function attach(seed: string): Promise<void> {
  ...
  await tauriInvoke("backend_attach", { seed });
  attached.add(seed);
}
...
export async function request<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  return tauriInvoke<T>("backend_request", { method, params });
}

export async function connect(): Promise<void> {
  await ensureBridgeListener();
  await tauriInvoke("backend_connect");
}

export async function backendStatus(): Promise<{ connected: boolean; error?: string }> {
  return tauriInvoke("backend_status");
}
...
export async function detachSession(seed: string): Promise<void> {
  ...
  await tauriInvoke("backend_detach", { seed });
}
