...
pub fn apply_bundle_zip(path: &Path, target: &Path, operation_id: &str) -> Result<BundleManifest> {
    ...
    if manifest.requires_full_install && !target.join("DeepX.exe").is_file() {
        return Err(UpdateError(format!(
            "{} bundle requires an existing DeepX installation at {}",
            manifest.kind,
            target.display()
        )));
    }
...
fn install_file(
    ...
    let temporary = parent.join(format!(".{file_name}.deepx-new-{}", std::process::id()));
    if temporary.exists() {
...
mod tests {
    ...
    fn component_bundle_preserves_previous_and_commits_v2_state() {
        ...
        let target = root.join("DeepX");
        fs::create_dir_all(target.join("resources")).unwrap();
        fs::write(target.join("DeepX.exe"), b"launcher").unwrap();
