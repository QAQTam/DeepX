...
pub fn write_install_root_marker(root: &Path) -> Result<()> {
    ...
    let temporary = canonical.join(".deepx-install-root.json.deepx-new");
...
mod tests {
    ...
    fn copied_marker_cannot_authorize_another_directory() {
        ...
        let source = root.join("DeepX-A");
        let destination = root.join("DeepX-B");
    ...
    fn test_root(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "deepx-install-root-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
}
