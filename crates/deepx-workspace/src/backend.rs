...
impl ToolExecutionBackend for HttpToolExecutionBackend {
    fn execute(&self, request: BackendRequest) -> ToolResult {
        ...
        match result {
            ...
            Err(error) => {
                // 仅"服务不可达"（连接被拒）回退本地：渐进式兼容
                // （serve 未启动时旧行为）。执行中的失败（读超时、断连、HTTP
                // 错误）绝不本地重跑——serve 端工具可能仍在执行，重跑会重复
                // 副作用（如 exec 的命令执行两遍、write 覆盖两次）。
                if matches!(&error, ureq::Error::ConnectionFailed) {
                    log::warn!(
...
mod tests {
    ...
    fn serve_bin_path() -> std::path::PathBuf {
        // 测试运行于 target/<profile>/deps/deepx_workspace-<hash>.exe；
        // serve 二进制在 target/<profile>/deepx-workspace(.exe)。
    ...
    fn spawn_serve() -> ServeGuard {
        ...
        let mut child = Command::new(exe)
            ...
            // serve_main 中 DEEPX_WORKSPACE_TOKEN 环境变量优先于 --token；
            // 测试进程可能继承 daemon 注入的变量，必须清除以保证 token 匹配。
            .env_remove("DEEPX_WORKSPACE_TOKEN")
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
            .expect("spawn deepx-workspace serve");
        ...
        for _ in 0..100 {
            ...
            if let Some(bound) = line.strip_prefix("DEEPX_WORKSPACE_READY ") {
                return ServeGuard {
                    child: Some(child),
