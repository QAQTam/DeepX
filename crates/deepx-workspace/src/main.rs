//! deepx-workspace binaries — CLI tool runner + HTTP tool service.
//!
//! Usage:
//!   deepx-workspace <tool_name> [json_args]
//!   deepx-workspace explore
//!   deepx-workspace read_file '{"path":"src/main.rs","start_line":1,"end_line":50}'
//!   deepx-workspace list
//!   deepx-workspace serve [--host 127.0.0.1] [--port 0] --token <secret>
...
fn main() {
    ...
    deepx_workspace::runtime::init_tools("cli", &[], vec![]);
    deepx_workspace::runtime::set_context("cli", 4);
    let cwd = env::current_dir()
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_else(|_| ".".into());
    deepx_workspace::workspace::set_process_workspace(&cwd);

    if args.len() < 2 {
        eprintln!("Usage: deepx-workspace <tool> [json_args]");
        eprintln!("       deepx-workspace list");
        std::process::exit(1);
    }

    let tool = &args[1];
    if tool == "list" {
        let defs = deepx_workspace::runtime::all_tools();
    ...
    let r = deepx_workspace::execution::execute_with_context(
        tool,
        "",
...
///
/// Token 来源（按优先级）：`DEEPX_WORKSPACE_TOKEN` 环境变量 > `--token` 参数。
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            other => {
                ...
                eprintln!("usage: deepx-workspace serve [--host H] [--port P] --token <secret>");
                std::process::exit(2);
            }
        }
        index += 1;
