//! TUI ↔ Agent frame definitions (channel-based, mpsc in-process).
//!
//! These were originally JSON-LP over stdin/stdout pipes but are now
//! pure Rust enums passed via `mpsc::Sender`/`Receiver`. Serde derives
//! are retained for backward-compatible headless mode.

use serde::{Deserialize, Serialize};

/// TUI → Agent frames (mpsc channel / stdin pipe in headless mode).
...
/// Agent → TUI frames (mpsc channel / stdout pipe in headless mode).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type")]
#[non_exhaustive]
pub enum AgentToTui {
