...
const mdComponents: Components = {
  ...
  pre({ children }) { return <pre className="bg-[#0d1117] p-3 rounded-lg overflow-x-auto text-[14px] leading-relaxed my-2 border border-[var(--border)]">{children}</pre> },
...
const ChatMessage = memo(function ChatMessage({ msg }: { msg: Message }) {
  ...
  const timeline = (() => {
    ...
    const segs = (msg as any).reasoningSegments || []
    const tools = msg.tool_cards || (msg as any).tool_calls || []
...
function ToolCard({ tc }: { tc: any }) {
  ...
  return (
    <div className={`rounded-xl border border-[var(--border)] overflow-hidden ${
      ...
      <div className={`flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-mono ${
        ...
        {tc.body?.command ? (
          <span className="truncate text-[var(--muted)] text-[14px]">$ {String(tc.body.command)}</span>
        ) : tc.args ? (
...
function ExecBody({ tc }: { tc: any }) {
  ...
  return (
    <div className="px-3 py-2 font-mono text-[14px] leading-relaxed text-[#e6edf3]">
      <div className="flex items-center gap-2 text-[#8b949e] mb-1">
        <span>$</span>
        <span>{cmd}</span>
      </div>
      {(() => {
        const display = tc.output || ''
        return display ? (
          <div className="whitespace-pre-wrap text-[14px] text-[#e6edf3] max-h-60 overflow-y-auto mt-2">
            <ResultViewer content={display} toolName={tc.name} maxLines={200} />
          </div>
        ) : (
          <div className="text-[13px] text-[#8b949e] mt-2">等待输出...</div>
        )
      })()}
    </div>
  )
...
function KVBody({ tc, icon }: { tc: any; icon?: string }) {
  const p = parseArgs(tc)
  if (!p) return <div className="px-3 py-2 text-[14px] font-mono text-[var(--muted)]">{safeArgsStr(tc).slice(0, 80)}</div>
  return (
    <div className="px-3 py-2 text-[14px] font-mono text-[var(--text)]">
...
function DiffFileBody({ tc }: { tc: any }) {
  ...
  if (tc.body?.old_lines && tc.body?.new_lines) {
    ...
    return (
      <div className="px-3 py-2 text-[14px] font-mono">
        {file && <div className="text-[var(--accent)] font-bold mb-1 text-[13px]">{file}</div>}
        <div className="grid grid-cols-2 gap-x-4 gap-y-0 text-[13px] leading-snug">
          <div className="text-[var(--muted)] text-[14px] mb-0.5">— 旧</div>
          <div className="text-[var(--muted)] text-[14px] mb-0.5">+ 新</div>
  ...
  return (
    <div className="px-3 py-2 text-[14px] font-mono">
      ...
      {oldLines.length > 0 || newLines.length > 0 ? (
        ...
        <div className="text-[var(--muted)] text-[13px]">{safeArgsStr(tc).slice(0, 120)}</div>
      )}
    </div>
  )
}
...
function AskBody({ tc }: { tc: any }) {
  const p = parseArgs(tc)
  return (
    <div className="px-3 py-2 text-[14px] text-[var(--text-h)]">
...
function ToolResult({ tc }: { tc: any }) {
  ...
  return (
    <details className="group border-t border-[var(--border)]">
      <summary className="flex items-center gap-1.5 px-3 py-1 text-[14px] font-mono text-[var(--muted)] cursor-pointer hover:text-[var(--accent)] select-none">
      ...
      <div className="max-h-64 overflow-y-auto bg-[var(--bg-tertiary)]/50 text-[var(--text)]">
        ...
        {hasTruncation && (
          <div className="px-3 py-1.5 text-[14px] font-mono text-[var(--warning)]/80 bg-[var(--warning)]/5 border-t border-[var(--border)]">
  ...
  return (
    <div className="font-mono leading-relaxed">
      {lines.map((line, i) => {
        ...
        return (
          <div key={i} className={`px-2 py-px text-[13px] ${cls}`}>
            {line || '\u00A0'}
          </div>
        )
      })}
      {total > showing && (
        <div className="px-2 py-1 text-[14px] text-[var(--muted)] italic border-t border-[var(--border)]">
