...
export function SettingsDialog({ onClose }: SettingsDialogProps) {
  ...
  const save = () => {
    invoke('save_config', { apiKey, baseUrl, model, contextLimit, maxTokens, autoMode, promptLang, effort }).then(() => {
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div>
            ...
            <input type="text" value={baseUrl} onChange={e => setBaseUrl(e.target.value)}
              className="w-full bg-[var(--bg-tertiary)] border border-[var(--border)] rounded-lg px-3 py-1.5 text-xs text-[var(--text-h)] font-mono outline-none focus:border-[var(--accent)]" />
            <div className="text-[9px] text-[var(--muted)] mt-1">DeepSeek Anthropic 兼容端点</div>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex items-end">
            <button onClick={fetchModels} disabled={fetching || !apiKey || !baseUrl}
        ...
        <div className="mb-4 p-3 rounded-lg border bg-[var(--bg-secondary)] border-[var(--border)]">
          <label className="flex items-center gap-3 cursor-pointer">
            <div className="flex-1">
              ...
              <div className="text-[10px] text-[var(--muted)]">自动选择模型和推理阶段</div>
            </div>
