...
| --- | --- | --- | --- |
| user prompt, reasoning/text fragments, tool position and terminal transcript state | `TimelineIntent` → `TimelineAppender` | v3 reliable timeline journal and one cursor | The renderer needs one causal order, not three channel-local orders. |
...
1. **Transcript projection group**: `Agent2Ui::{TurnStart, RoundDelta,
   ...
   after v3 Timeline SSE and the Electron timeline reducer are the only desktop
   transcript source.
...
3. **Replay group**: `EventBus` Agent2Ui projection and `session.replay_events`.
   Delete after persisted v3 timeline/control snapshots cover resume and load
   more turns.
...
- Do not use a renderer-side merge of v2 `conversation` and `tool` streams.
- Do not silently accept a v2 client on a v3-only endpoint. Version negotiation
...
`Timeline_intent_v3`. The runtime's `RingingHub::publish_timeline` owns the
...
fetches `/ringing/v3/sessions/{seed}/timeline` and consumes one per-session
SSE cursor; its reducer rejects gaps and renders Markdown only after a block
seal.

This is still a staged cutover: v2 control stores supply interactions,
